...
/// Button翻页管理类
/// </summary>
public class GridPageView : MonoBehaviour
{
    //ButtonItem容器
    ...
    //更新的回调方法
    public delegate void UpdateGridItem(GameObject item, int itemIndex, int showPageIndex, string text,float value, bool isReload);
    //更新列表回调方法
                     ...
                     UpdateGridItem updateItem = null)
    {
        ...
        //一页多少格子
        cellsMaxCountInPage = rows * columns;
        ...
        //计算单页高宽
        pageSize = new Vector2(this.columns * (itemWidth + this.gapH),
                                    this.rows * (itemHeight + this.gapV));

        scroll.GetComponent<RectTransform>().sizeDelta = pageSize;
        sr = scroll.GetComponent<ScrollRect>();
        sr.horizontal = this.isHorizontal;
        sr.vertical = !this.isHorizontal;
        contentRectTf = content.GetComponent<RectTransform>();
        content.transform.localPosition = new Vector3(0, 0);
        firstPagePos = new Vector2();
        reloadData(cellCount);
    }
    ...
    private void createPageItem(GameObject prefab, int createPageCount)
    {
        ...
        for (int i = 0; i < createPageCount; ++i)
        {
            ...
            {
                GameObject item =
                    MonoBehaviour.Instantiate(prefab, new Vector3(0, 0), new Quaternion()) as GameObject;
        ...
        for (int i = this.curPageIndex; i < this.curPageIndex + this.showPageCount; ++i)
        {
            if (this.pageItemList[pageIndex] != null )
            {
                ...
                {                                        
                    ...
                    {
                        string text = dataManager.tableArray[itemIndex, 0].ToString();
                        float value = float.Parse(dataManager.tableArray[itemIndex, 1].ToString());
                        GameObject item = itemList[j];
                        if (this.m_updateItem != null)
                        {
                            //(GameObject item, int index, int pageIndex, string text, float value, bool isReload);
                            this.m_updateItem.Invoke(item, itemIndex,i,text,value,isReload);
                        }
                    }
                }
                pageIndex++;
    ...
    public void rollPosByPage(int page)
    {
        ...
        //    this.curPageIndex -= this._pageCount - this.showPageCount;
        ...
        else
        {
            ...
            contentPos.x = -this.pageSize.x * (page-1);
        }
